...
import { Plus, Search, MapPin, Trash2, GraduationCap, Zap, FileSearch, Sparkles, AlertCircle } from 'lucide-react';
      ...
      {filteredJobs.some(j => j.status === 'Applied') && (
        <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl flex items-center gap-3 animate-pulse">
           ...
           <p className="text-xs text-amber-800 font-bold">
              Some applications are <span className="underline">Unoptimized</span>. Use <span className="bg-purple-100 px-1.5 py-0.5 rounded text-purple-700">Magic Apply</span> to optimize your resume instantly.
           </p>
        </div>
      )}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50">
                <th className="px-6 py-4 text-xs font-black text-slate-400 uppercase tracking-wider">Position & Company</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredJobs.map(job => (
                <tr key={job.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${getStatusColor(job.status)}`}>
                      {job.status}
                    </span>
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                      {job.status === 'Auto-Pilot' ? (
                        <button 
                          ...
                          className="flex items-center gap-1.5 px-4 py-2 bg-slate-900 hover:bg-black text-white rounded-xl text-xs font-bold transition-all shadow-lg"
                        >
                        ...
                        <button 
                          ...
                          className="group/btn flex items-center gap-1.5 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold transition-all shadow-lg animate-pulse hover:animate-none"
                        >
                          <Zap className="w-3.5 h-3.5 fill-current group-hover/btn:scale-125 transition-transform" />
                          Magic Apply
                        </button>
                      )}
